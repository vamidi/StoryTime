...
namespace DatabaseSync
{
	...
	public class DatabaseSyncModule
	{
		...
		private static void Login(Action callback)
		{
			...
			if (string.IsNullOrEmpty(configFile.password) && string.IsNullOrEmpty(configFile.password))
			{
		...
		private static DatabaseConfig Fetch()
		{
		...
		private void OnResponseReceived(UnityWebRequest request, string tableID = "")
		{
			...
			else
			{
				...
				{
					Table tableData = item.ToObject<Table>() ?? throw new ArgumentException($"Can't make Table from JSON file");
